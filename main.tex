...
\begin{document}
    ...
    \subsection{Application to fibers of nested sets}
        ...
        where $\mathcal{N}(\mathcal{L}, \mathcal{G})$ is the set of nested sets with respect to the building set $\mathcal{G}$, called extended support map, as follows
        ...
        Since we need to construct matrices we establish here some notation.
        ...
        \end{bmatrix}  \]
        explicitly
        \[ M^{\gamma_i}_{F, k}(i,j) = \gamma(j)x_F^{i}\]
        ...
        \[ M^{\gamma}_{E, k} = 
            ...
            \end{bmatrix}  \]
        explicitly
        \[ M^{\gamma_i}_{E, k}(i,j) = \gamma(j)x_E^{i}\]
    ...
    Consider a chain $\gamma$, $F \in \mathcal{L} \setminus \{E\}$ and $k > 0$. Then the aim is to count the number of symmetric chains obtainable by the matrix
    \[ M^{\gamma}_{F, k}.\]
    ...
    \begin{enumerate}
        ...
        Therefore the number of chains is equal to the number of rows or columns
        \[ L(\gamma) = k-1.\]
        ...
        \begin{center} \begin{tikzpicture}[baseline=0cm,mymatrixenv]
                ...
                \end{tikzpicture} \end{center}
            So the number of chains is equal to the number of rows, which is
            \[ k-1.\]
            ...
            \item[$L(\gamma) < k-1$:] in this case the matrix is vertical. Called $l = L(\gamma)$:
                ...
                Therefore in this case the number of chains is equal to the number of columns, which is
                \[ L(\gamma).\]
    \end{enumerate}

    So in the end we proved the following
    \begin{thm}[Number of chain in a chain matrix]
    ...
    In this example we redo all the process described previously for the case $N=\{F_1\}$, and so $N^+=\{F_1, E\}$.
    ...
    \[ l + (k-2)\]
    where $l$ is the number of elements in the first row, and $k-2$ is the number of elements in the last column, except for the first element.
    ...
    By applying the analogous argument with the final chain $m^{\gamma}_{E, k}$ we obtain the following theorem.
    \begin{thm}
        Consider a chain $\gamma$, with $l = L(\gamma)$, $F\in \mathcal{L} \setminus \{E\}$ and $k > 0$. Then the length of the chain in $M^{\gamma}_{F,k}$ are
        \[ L(\gamma_i) = l + (k-2) - 2(i-1);\]
        and the length of the chains in $M^{\gamma}_{E,k}$ are
        \[ L(\gamma_i) = l + (k-1) -2(i-1).\]
    \end{thm}

    Consider now a first chain $\gamma_1$, and some elements $F_1, \dots, F_l \in \mathcal{L}\setminus \{ E\}$ with some natural number $k_1, \dots, k_l$.
        ...
        \begin{enumerate}
            ...
            \[ \gamma^2_{1}, \dots ,\gamma^2_{N_2},\]
            where the exponents is the number of the step we are in.
            Therefore the length of these chains, by the theorem, is
            \[ L(\gamma^2_i) = L(\gamma_1) + (k_1 -2) - 2(i-1)\]

            \item[Step 3:] consider the matrices
            \[ M^{\gamma^2_1}_{F_2, k_2}, \dots, M^{\gamma^2_{N_2}}_{F_2, k_2}.\]

            And we extract, for every matrix $M^{\gamma_i^2}_{F_2, k_2}$, the chains
            \[ \gamma^{3,i}_{1}, \dots, \gamma^{3,i}_{N_{3,i}},\]
            where the exponent is the number of the step we are in and t, and $N_{3,i}$ is the number of chain extracted by the $i-th$ matrix $M^{\gamma^2_i}_{F_2,k_2}$.

            Consider the $i-th$ matrix, by the theorem the length of the chains is
            \begin{align*}
                L(\gamma^{3,i}_k) &= L(\gamma^2_i) - (k_2 -2) - 2(k-1) =\\
                &= L(\gamma_1) + (k_1 - 2) - 2(i-1) + (k_2 -2) - 2(k-1) =\\
                &= L(\gamma_1) + (k_1 -2) + (k_2 -2) - 2(i-1) -2(k-1).
            \end{align*}

            \item[Step s:] let $t_1, \dots, t_{s-1}, t_s$ be the index of the chosen chains in every step.
            Suppose it is true that
                \[ L(\gamma^{s,t_{s-1}}_{t_s}) = L(\gamma_1) + \sum_{j = 1}^{s-1} (k_j -2) - 2\sum_{j=1}^{s-1}(t_j -1).\]

            \item[Step s+1:] consider $t_1, t_2, \dots, t_s$ be the indexed of the choosen chains in the first $s$ steps of the process. Then consider the chains extracted by the last step as:
            \[ \gamma_1^{s, t_s}, \dots, \gamma^{s, t_s}_{N_{s,t_s}} \]
            and consider the $t_{s+1} \in \{1, \dots, N_{s,t_s}\}$. Then by the theorem
            \[ L(\gamma_{t_{s+1}}^ {s+1, t_{s}}) = L(\gamma_{t_s}^{s,t_{s-1}}) + (k_{s} -2) - 2(t_{s}-1).\]
            But by inductive hypothesis we have that
            \[ L(\gamma^{s,t_{s-1}}_{t_s}) = L(\gamma_1) + \sum_{j = 1}^{s-1} (k_j -2) - 2\sum_{j=1}^{s-1}(t_j -1),\]
            therefore
            \begin{align*}
                L(\gamma_{t_{s+1}}^ {s+1, t_{s}}) &= L(\gamma_1) + \sum_{j = 1}^{s-1} (k_j -2) - 2\sum_{j=1}^{s-1}(t_j -1) + (k_{s} -2) - 2(t_{s}-1)\\ &= L(\gamma_1) + \sum_{j = 1}^{s} (k_j -2) - 2\sum_{j=1}^{s}(t_j -1)
            \end{align*} 
        \end{enumerate}   
    ...
    We give here a Lemma to compute also the length o the chains in the decomposition of $supp_+^{-1}(N^+)$.
    \begin{lem} \label{lem:lengthF_1}
        ...
        \[ L(\gamma_i) = L(\gamma_1) - 2(i-1);\]
        for $1 \le i \le K$ and where $\gamma_1$ is the most external, and the others are progressively the more internals.
    \end{lem}
...
\item[$N=\{F_1, F_2\}$] We consider now the case $N=\{F_1, F_2 \}$.
    \begin{prop}
        If $N = \{F_1, F_2 \}$, then
    ...
    \end{proof}
        ...
        \[ M^{\gamma_i}_{E, m_E} \]
        fore every $1 \le i \le K$, and count the number of chains in every matrix.

        \begin{enumerate}
            \item[i$=1$:] to compute the number of chains in $M^{\gamma_i}_{E, m_E}$ we observe that the number of row in equal to the length of $\gamma_1$ and then number of column is $m_E$, so the number of chains is
            \[ N_1 = \min \{ m_1 + m_2 -3, m_E\}.\]

            \item[$i=2$:] in this case the number of row is equal to the length of $\gamma_2$ that is 
            \[ L(\gamma_2) = L(\gamma_1) - 2 = m_1 + m_2 -3 -2 = m_1 + m_2 - 5, \]
            and the number of column is $m_E$.
            So the number of chains is
            \[ \min \{ m_1 +m_2 - 5, m_E \}. \]

            \item[$i>2$:] we can generalize by observing that the number of chains in a matrix is the minimum between the number of rows and the number of column.
            So 
            \[N_i = \min \{ m_1 + m_2 - 2i -1  , m_E \}. \]
        \end{enumerate}

        So in the end we have the theorem
        \begin{thm}
            Consider $N=\{F_1, F_2 \}$, let $K = SC(D(x_{F_1}^{m_1-1}x_{F_2}^{m_2-1}))$ and $\gamma_1, \dots, \gamma_K$ the chains of the decomposition of $SC(D(x_{F_1}^{m_1-1} x_{F_2}^{m_2-1}))$. Then for $1 \le i \le K$, the number of chains in 
            \[ M^{\gamma_i}_{E, m_E}\]
            is equal to $N_i = \min \{ m_1 + m_2 - 2i -1, m_E \}$,
            and called its chains $\phi^i_{1}, \dots, \phi^i_{N_i}$, their length are
            \[ L(\phi^i_{N_i}) = m_1 + m_2 + m_E - 2i -1. \]
        \end{thm}

Consider now the general case $N = \{F_1,\dots, F_l\}$, and define
    \[ K = SC(D(x_{F_1}^{m_1-1}\cdots x_{F_{l-1}}^{m_{l-1}-1}))\]
    and $\gamma_1, \dots, \gamma_K$ the chains in the decomposition of $D(x_{F_1}^{m_1-1}\cdots x_{F_{l-1}}^{m_{l-1}-1})$.
    Suppose that the length of this chains are
    \[ L(\gamma_i) = m_1 + \dots + m_{l-1} - 2(l-2) -1 - 2(i -1),\]
    for $1 \le i \le K$.
    Then for all $1 \le i \le K$, we construct the matrix
    \[ M^{\gamma_i}_{F_l, m_l}.\]
    By hypothesis the number of column is $m_1 + \dots + m_{l-1} - 2(l-2) -1 - 2(i -1)$ and the number of rows is $m_l -1$, so the number of chains in $M^{\gamma_i}_{F_l, m_l}$ is
    \[ N_i = \min \{ m_1 + \dots + m_{l-1} - 2(l-2) - 2(i -1), m_l \} -1.\]
    
    Now we compute the length of these chains. The number of elements in the first chains is the number of element in the first row added to the number of elements in the last column, except the first element, so in $M^{\gamma_1}_{F_l, m_l}$
    \[ L(\phi^{1}_1) = m_1 + \dots + m_{l-1} - 2(l-2) -1 + (m_l -2) = m_1 + \dots + m_{l} - 2(l-1) -1.\]
    The length of the second chain is
    \[ L(\phi_2^1) = L(\phi^1_1) -2 = m_1 + \dots + m_{l} - 2(l-1) -1 -2. \]
    Therefore, in general we have
    \[ L(\phi_i^1) = L(\phi^1_1) - 2(i-1) = m_1 + \dots + m_{l} - 2(l-1) -1 -2(i-1). \]

    Consider now the chain $\gamma_j$, then 
    \[ L(\phi^s_1) = m_1 + \dots + m_{l} - 2(l-1) -1 -2(i-1)\]
...
\newpage
    ...
    To construct the base for $P^k$ we proceed step by step. Consider the nested set $N = \{ F_1, \dots, F_l\}$, therefore $N^+=\{F_1, \dots, F_l, E\}$.
    ...
    \begin{enumerate}
        ...
        By theorem ... the number of chain is \[N_{2,1} = \min \{ L(\gamma_1), m_2 -1 \} = \min \{ m_1, m_2 \} -1.\] So the exponents of $x_{F_2}$ in the first elements of the chains ranges in $\{1, \dots, N_{1}\}$, and the first elements are
        ...
        \item[Step 3:] choose $t_2 \in \{1, \dots, N_{2,1}\}$, and construct the matrix
        \[ M^{\gamma^{1}_{t_2}}_{F_3, m_3}.\]
        
        By theorem ... we have that the number of chains is
        \[ N_{1, t_2} = \min \{ L(\gamma^{2,1}_{t_2}), m_3 -1 \} = \min \{ L(\gamma_1) +(m_2 -2) - 2(t_2 -1), m_3 -1 \}\]
        so the exponents of $x_{F_3}$ ranges in $\{1, \dots, \min \{ L(\gamma_1) +(m_2 -2) - 2(t_2 -1), m_3 -1 \}$, and the first elements of the chains are
        \[ \{ x_{F_1}x_{F_2}^{t_2} x_{F_3}^{t_3} : 1 \le t_3 \le \min \{ L(\gamma_1) +(m_2 -2) - 2(t_2 -1), m_3 -1 \} \}.\]
        By letting $t_2$ ranging in $1, \dots, \min \{m_1, m_2 \} -1$, we have
        \[ \{ x_{F_1}x_{F_2}^{t_2} x_{F_3}^{t_3} : 1 \le t_2 \le \min \{m_1, m_2 \}-1 , 1 \le t_3 \le \min \{ L(\gamma_1) +(m_2 -2) - 2(t_2 -1), m_3 -1 \} \} \]
        ...
        but by theorem ... we have
        \[ L(\gamma^{1, t_2}_{t_3}) = L(\gamma_1) + (k_2 -2) + (k_3 -2) - 2(t_2 -1) - 2(t_3-1), \]
        so is
        \[ \min \{ L(\gamma_1) + (k_2 -2) + (k_3 -2) - 2(t_2 -1) - 2(t_3-1), m_4-1 \}. \]

        Call the chains extracted by this matrix
        \[ \gamma_1^{1,t_2,t_3}, \dots, \gamma_{N_{1,t_2,t_3}}^{1,t_2,t_3},\]
        and observe that whe have the correspondence
        \[ x_{F_1}x_{F_2}^{t_2}x_{F_3}^{t_3}x_{F_4}^{t_4} \longleftrightarrow \gamma^{1,t_2,t_3}_{t_4}.\]

        \item[Step s:]
    \end{enumerate}
    
    \section{Decomposition of $A^k$}
    In this section we want to decompose $A^k$ using the previous results.
    We want to prove the following
    \begin{thm}
        Given $k$, then
        \[ A^k = P^k \oplus \lambda(A^{k-1})\]
    \end{thm}
