...
\newpage
    ...
    To construct the base for $P^k$ we proceed step by step.

    \begin{enumerate}
        \item[$N^+=\{F_1, E\}$:]By the previous theorem the primitive elements are all the first elements of the chains.

        The number of this elements is equal to the number of chains in the decomposition, which is equal to
        \[ N = \min \{m_{N ^+}(F_1) -1, m_{N^+}(E)\} \]
        
        Therefore, the nested sets of the type $N^+=\{F_1, E\}$, contribute to the basis with the elements
        \[ \{ x_{F_1}x_E^k \mid 0 \le k \le \min \{m_{N ^+}(F_1) -1, m_{N^+}(E)-1\} \}. \]

        \item[$N^+ = \{ F_1, F_2, E\}$:] in this case the we first compute the first elements of the chains in $D(x_{F_1}^{m_1-1} x_{F_2}^{m_2-1})$.
        The number of chains in $M^\gamma_{F_2, m_2}$, where $\gamma$ is the only chain in the decomposition of $x_{F_1}^{m_1-1}$ is 
        $N _1 = \min \{ m_{N^+}(F_1) , m_{N^+}(F_2)\}-1$, so the first elements of these chains are
        \[ \{ x_{F_1}x_{F_2}^k, \, 1 \le k \le \min \{ m_{N^+}(F_1) , m_{N^+}(F_2)-1\} .\]
        If we call $\gamma_1, \gamma_2, \dots, \gamma_{N_1}$ the chains in the decomposition of $x_{F_1}^{m_1-1} x_{F_2}^{m_2-1}$, then we observe that the exponent of $x_{F_2}$ is in correspondence to the index of the chain:
        \[ x_{F_1}x_{F_2}^k \longleftrightarrow \gamma_k, \]
        so exponent of $x_{F_2}$ in the first element of $\gamma_k$ is $k$.
    
        Therefore, since for every $1 \le i \le N_1$ we want to construct the matrix
        \[ M^{\gamma_i}_{E, m_E}\]
        we have also the correspondence
        \[ x_{F_1}x_{F_2}^k \longleftrightarrow M^{\gamma_k}_{E, m_E}\]

        So, given $k \in [1; N_1]$, we have that the number of chains in the matrix $M^{\gamma_k}_{E, m_E}$ is 
        \[ N_E = \min \{ L(\gamma_k), m_E\}, \]
        where $L(\gamma_k) = m_1 + m_2 - 2(k-1) -1$.
        Hence the first elements in the chains of this matrix are
        \[ \{x_{F_1} x_{F_2}^{k} x_{E}^{t}  \mid 0 \le t \le \min \{ m_1 + m_2 - 2k, m_E \} -1\}.\]
        By letting $k$ ranging in $[1; \min \{ m_{N^+}(F_1) , m_{N^+}(F_2)\}-1]$, we have that the first element of the chains are
        \[ \{ x_{F_1} x_{F_2}^k x_{E}^t  \mid 1 \le k \le \min \{ m_1, m_2 \} -1, 0 \le t \le \min \{ m_1 + m_2 - 2k, m_E \} -1\}. \]

        \item[$N = \{ F_1, \dots, F_l\}$:]
        suppose that the first elements of the chains in the decomposition of $D(x_{F_1}^{m_1 -1} \cdots x_{F_{l-1}}^{m_{l-1}-1})$ are
        \begin{center}
        $\left \{x_{F_1} x_{F_2}^{t_2} \cdots x_{F_{l-1}}^{t_{l-1}} :
        \begin{array}{c}
            1 \le t_2 \le \min \{ m_1, m_2\}-1\\
            \vdots\\
            1 \le t_{l-1} \le \min \{ m_1 + \dots + m_{l-2} - 2(l-3) - 2t_{l-2}, m_{l-1}\} -1 \}
        \end{array} \right \}$
        \end{center}

        We call $\gamma_1, \dots, \gamma_{N_{l-1}}$ the chain in the decomposition of $D(x_{F_1}^{m_1 -1}) \cdots x_{F_{l-1}}^{m_{l-1}-1})$, then we observe that for every fixed $(k_2, \dots, k_{l-2})$, there exists a correspondence
        \[ x_{F_1} x_{F_2}^{k_2} \cdots x_{F_{l-2}}^{k_{l-2}} x_{F_{l-1}}^k  \longleftrightarrow \gamma_k \longleftrightarrow M^{\gamma_k}_{F_l, m_l}.\]
        Therefore, by choosing $k \in [1, N_{l-1}]$ we are choosing a chain $\gamma_k$ with which we will construct the matrix of chains
        \[ M^{\gamma_k}_{F_l, m_l}.\]
        In this matrix we construct the chains as before and the number of chains is
        \[ \min \{ L(\gamma_k), m_{l} \} -1, \]
        where by inductive hypothesis $L(\gamma_k) = m_1 + m_2 + \dots + m_{l-1} - 2(l-2) -2(k-1) -1$, so the number of chains is
        \[ \min \{ m_1 + m_2 + \dots + m_{l-1} - 2(l-2) -2(k-1) , m_l \} -1. \]
        By letting $k$ ranging in $[1, N_{l-1}]$, then the first elements of the chains are
        \[
        \{x_{F_1} x_{F_2}^{t_2} \cdots x_{F_{l}}^{t_l} :
            1 \le t_k \le \min \{ m_1 + \dots + m_{k-1} - 2(k-2) - 2(t_{k-1}-1), m_k\} -1 \}\]
        The last step is to add the element $E$. As before, for $k \in [1, N_l]$ we are choosing the chain $\gamma_k$ to work with. So we can construct the matrix $M^{\gamma_k}_{E, m_E}$, and construct the chains. The number of chains is 
        \[ N_E = \min \{ m_1 + \dots + m_l - 2(l-1) - 2(k-1) -1, m_E \},\]
        so the first element of the chains are
        \begin{center}
        $\left \{x_{F_1} x_{F_2}^{t_2} \cdots x_{F_{l}}^{t_l} x_E^{t_E} :
        \begin{array}{c}
            1 \le t_k \le \min \{ m_1 + \dots + m_{k-1} - 2(k-2) - 2(t_{k-1}-1), m_k\} -1 \\
            0 \le t_E \le \min \{ m_1 + \dots + m_l - 2(l-1) - 2(t_l-1), m_E \} -1
        \end{array} \right \}$
        \end{center}
    ...
    We use now to above results to decompose $A^k$.
