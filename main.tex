...
\begin{document}
    ...
    Here we want to count the number of chains in the decomposition of a fiber and their length.
    We first proceed by computing the case $N = \{F_1 \}$ and $N=\{F_1, F_2 \}$, then we generalize the results.
    ...
    \begin{enumerate} 
        \item[$N=\{F_1\}$] We start by considering the most simple case $N=\{F_1\}$, and so $N^+=\{F_1, E\}$.
...
\item[$N=\{F_1, F_2\}$] We consider now the case $N=\{F_1, F_2 \}$.
    ...
    \item[$N=\{F_1, \dots, F_l\}$] Consider now the general case $N = \{F_1,\dots, F_l\}$, and define
    ...
    \[ L(\gamma_i) = m_1 + \dots + m_{l-1} - 2(l-2) + 2(i -1) -1,\]
    for $1 \le i \le K$.
    Then for all $1 \le i \le K$, we construct the matrix
    \[ M^{\gamma_i}_{F_l, m_l}.\]
    By hypothesis the number of column is $m_1 + \dots + m_{l-1} - 2(l-2) + 2(i -1) -1$ and the number of rows is $m_l -1$, so the number of chains in $M^{\gamma_i}_{F_l, m_l}$ is
...
\newpage
    ...
    Consider now the nested set $N^+ = \{ F_1, F_2, E\}$. The first elements of the chains in the decomposition of $p=x_{F_1}^{m_1-1} x_{F_2}^{m_2-1}$ are the first $N$ elements in the first row, where $N = \min \{ m_{N^+}(F_1) , m_{N^+}(F_2)\}-1$
    \[ \{ x_{F_1}x_{F_2}^k, \, 1 \le k \le \min \{ m_{N^+}(F_1) , m_{N^+}(F_2)-1\} .\]

    If we call $\gamma_1, \dots, \gamma_N$ the chains in the decomposition of $p$ then we observe that the exponent of $x_{F_2}$ is equal to the index of the chain:
    \[ x_{F_1}x_{F_2}^k \longleftrightarrow \gamma_k, \]
    so exponent of $x_{F_2}$ in the first element of $\gamma_k$ is $k$.

    Now, the first elements of the whole decomposition are obtained by following the procedure described above and considering the first elements of the chains.
    Take the $k$-th chain of the decomposition, and consider the matrix obtained by multiplying the selected chain by $x_E^i$ and consider the first $N$ elements of the first column, where
    \[ N = \min \{ (m_1-1)+(m_2-2) - 2(k-1) -1, m_E \}, \]
    then the elements of the basis are
    \[ \{ x_{F_1} x_{F_2}^k x_{F_3}^j  \mid 1 \le k \le \min \{ m_1, m_2 \} -1, 0 \le j \le \min \{ (m_1 -1)+(m_2 -2) - 2(k-1), m_E \}-1 \}. \]

    Let us consider the case $N = \{ F_1, \dots, F_k\}$. Then by inductive hypothesis he decomposition of $N^- = \{ F_1, \dots, F_{k-1}\}$ is
    \[ \{ x_{F_1} x_{F_2}^{t_2} \cdots x_{F_{k-1}}^{t_{k-1}} \mid 1 \le t_j \le \min \{\sum_{1 \le l \le j-1} m_{N^+}(F_l) - 2 (t_{j-1} -1), m_{N^+}(F_{j-1})\}-1 \}. \]

    !! DA SISTEMARE UN SACCO DI COSE !!
        
    So by using the procedure as before we multiply every chain by $x_{F_k}^i$ where $i$ is the index of the row minus one. The number of chains is
    \[ \min \{ \sum_{1 \le l \le k-1} m_{N^+}(F_l) - 2(i-1), m_{N^+}(F_k) -1 \}, \]
    but the index $i$ of the row, is equal to the index of the chains, which is equal to the exponent of $x_{F_{k-1}}$. So the contribution of $N$ to the basis are
    \[ \{ x_{F_1} x_{F_2}^{t_2} \cdots x_{F_{k}}^{t_k} \mid 1 \le j \le k, \,  1 \le t_j \le \min \{\sum_{1 \le l \le j-1} m_{N^+}(F_l) - 2 (t_{j-1} -1), m_{N^+}(F_{j-1})\}-1 \}. \]
    Next we add $x_E$. We multiply every chain for $x_E^{i-1}$, then the number of chains of the new matrix is
    \[ \min \{ \sum_{1 \le l \le k-1} m_{N^+}(F_l) - 2(i-1), m_{N^+}(E)\} -1 \]
    so the base is 
    \[ \{ x_{F_1} x_{F_2}^{t_2} \cdots x_{F_{k}}^{t_k} x_E^{t_E} \mid 1 \le j \le k, \,  1 \le t_j \le \min \{\sum_{1 \le l \le j-1} m_{N^+}(F_l) - 2 (t_{j-1} -1), m_{N^+}(F_{j-1})\}-1, \, 0 \le t_E \le \min \{ \sum_{1 \le l \le k} m_{N^+}(F_l) - 2(t_k-1), m_{N^+}(E)\} -1 \}. \]
